use super::smoke::{
    ...
    CAP_AUDIT_MARKER, CAP_AUDIT_STATUS_MARKER, CAP_IPC_AUDIT_MARKER, CAP_IPC_FAIL_MARKER,
    CAP_IPC_GRANT_MESSAGE_MARKER, CAP_IPC_MARKER, CAP_IPC_RECEIVER_READ_MARKER,
    CAP_IPC_RECEIVER_REVOKED_MARKER, CAP_IPC_RECEIVER_WRITE_DENIED_MARKER,
    CAP_IPC_REGISTRY_EPOCH_MARKER, CAP_IPC_REVOKE_MESSAGE_MARKER,
    CAP_IPC_SENDER_GRANT_DENIED_MARKER, CAP_IPC_STATUS_MARKER, CAP_TABLE_FAIL_MARKER,
    CAP_TABLE_MARKER, CAP_TABLE_STATUS_MARKER, CONCURRENCY_FAIL_MARKER,
    CONCURRENCY_IRQ_LOCK_MARKER, CONCURRENCY_IRQ_MARKER, CONCURRENCY_LOCK_MARKER,
    CONCURRENCY_MARKER, CONCURRENCY_NESTED_IRQ_MARKER, CONCURRENCY_ORDER_MARKER,
    CONCURRENCY_STATUS_MARKER, COOPERATIVE_SCHED_FAIL_MARKER, COOPERATIVE_SCHED_MARKER,
    COOPERATIVE_SCHED_STATUS_MARKER, CPU_HARDENING_ARCH_CAPABILITIES_MARKER,
    CPU_HARDENING_FAIL_MARKER, CPU_HARDENING_IBPB_ATTEMPTED_MARKER, CPU_HARDENING_IBPB_MARKER,
    ...
    IPC_PINGPONG_BACKPRESSURE_MARKER, IPC_PINGPONG_FAIL_MARKER, IPC_PINGPONG_MARKER,
...
fn qemu_markers_track_current_contracts() {
    ...
    assert_eq!(CAP_IPC_STATUS_MARKER, "cap-ipc grant_seq=");
    assert_eq!(
        CAP_IPC_GRANT_MESSAGE_MARKER,
        "cap_ipc_grant_message_ok=true"
    );
    assert_eq!(
        CAP_IPC_RECEIVER_READ_MARKER,
        "cap_ipc_receiver_read_ok=true"
    );
    assert_eq!(
        CAP_IPC_RECEIVER_WRITE_DENIED_MARKER,
        "cap_ipc_receiver_write_denied=true"
    );
    assert_eq!(
        CAP_IPC_SENDER_GRANT_DENIED_MARKER,
        "cap_ipc_sender_missing_grant_denied=true"
    );
    assert_eq!(
        CAP_IPC_REVOKE_MESSAGE_MARKER,
        "cap_ipc_revoke_message_ok=true"
    );
    assert_eq!(
        CAP_IPC_REGISTRY_EPOCH_MARKER,
        "cap_ipc_registry_epoch_bumped=true"
    );
    assert_eq!(
        CAP_IPC_RECEIVER_REVOKED_MARKER,
        "cap_ipc_receiver_revoked=true"
    );
    assert_eq!(CAP_IPC_AUDIT_MARKER, "cap_ipc_grant_audit_seen=true");
    assert_eq!(CAP_IPC_FAIL_MARKER, "[TEST] cap-ipc=fail");
    assert_eq!(CAP_IPC_MARKER, "[TEST] cap-ipc=ok");
    ...
    assert_eq!(CPU_HARDENING_IBPB_ATTEMPTED_MARKER, "ibpb_attempted=");
