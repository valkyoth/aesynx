...
use super::smoke::{
    ...
    MULTICORE_TOPOLOGY_STARTUP_EVIDENCE_MARKER, MULTICORE_TOPOLOGY_STATUS_MARKER,
    ...
    PAGE_TABLE_EXECUTABLE_RANGE_MARKER, PAGE_TABLE_FLAGS_MARKER, PAGE_TABLE_FLUSH_PAGE_MARKER,
...
fn manifest_text_field(name: &str, value: &str) -> Result<String, String> {
    if value.contains('\n') || value.contains('\r') {
        return Err(format!("{name} contains newline or carriage return"));
    }
    Ok(value.to_owned())
}
