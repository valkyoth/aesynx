...
| Page-table mapping | Safe bounded mapper model validates canonical/aligned addresses, rejects double maps, maps/unmaps/translates 4 KiB pages, carries generic page flags, and returns explicit TLB flush targets before any live CR3 activation claim | Active candidate | `aesynx-mm::PageTableMapper`, `cargo xtask qemu` |
