...
| Early diagnostics | Structured log-level records with validated components, escaped and bounded panic output, boot phase, early core, source location, message, redacted register summary, and QEMU panic marker | Active candidate | `aesynx-kernel::diagnostics`, `aesynx-arch-x86_64::registers`, `cargo xtask qemu --panic-smoke` |
...
| QEMU testing | Boot smoke validates BootInfo and Rust serial markers; panic smoke validates early panic diagnostics | Active for boot | `cargo xtask qemu`, `cargo xtask qemu --panic-smoke`, `docs/RELEASE_PLAN.md` |
