...
| Monotonic time and sleep queue | Timer ticks convert into checked monotonic instants; bounded sleep requests wake only when due; timeout checks fail closed on overflow | Active candidate | `aesynx-time`, `cargo xtask qemu --timer-smoke` |
...
| QEMU testing | Boot smoke validates GDT/TSS/IDT setup, breakpoint handling, BootInfo, and Rust serial markers; panic smoke validates early panic diagnostics; exception smoke validates page-fault decoding; timer smoke validates three controlled PIT IRQ0 ticks plus delayed sleep wake markers | Active for boot | `cargo xtask qemu`, `cargo xtask qemu --panic-smoke`, `cargo xtask qemu --exception-smoke`, `cargo xtask qemu --timer-smoke`, `docs/RELEASE_PLAN.md` |
