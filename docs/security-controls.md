...
| Page-table mapping | Safe bounded mapper model validates canonical/aligned addresses, rejects double maps, keeps device/cache flags behind builders/accessors, maps/unmaps/translates 4 KiB pages, exposes read-only permission lookup, changes mapping permissions without changing frames, carries generic page flags, and returns explicit TLB flush targets before any live CR3 activation claim | Active candidate | `aesynx-mm::PageTableMapper`, `cargo xtask qemu` |
