...
| Frame allocation | Early bitmap allocator tracks known/free/used/reserved frame state, rejects overlapping seeds, detects double-free, validates region seeding and contiguous frees before committing bitmap mutations, rejects impossible private bitmap combinations on checked status reporting, avoids unchecked status arithmetic underflow, and is QEMU-smoked against a bounded usable boot-map window | Active candidate | `aesynx-mm::BitmapFrameAllocator`, `cargo xtask qemu` |
