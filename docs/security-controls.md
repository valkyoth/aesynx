...
| Frame allocation | Early bitmap allocator tracks known/free/used/reserved frame state, rejects overlapping seeds, detects double-free, and is QEMU-smoked against a bounded usable boot-map window | Active candidate | `aesynx-mm::BitmapFrameAllocator`, `cargo xtask qemu` |
...
| QEMU testing | Boot smoke validates GDT/TSS/IDT setup, breakpoint handling, BootInfo, memory-map accounting, bounded frame-allocator smoke, and Rust serial markers; panic smoke validates early panic diagnostics; exception smoke validates page-fault decoding; timer smoke validates three controlled PIT IRQ0 ticks plus delayed sleep wake markers | Active for boot | `cargo xtask qemu`, `cargo xtask qemu --panic-smoke`, `cargo xtask qemu --exception-smoke`, `cargo xtask qemu --timer-smoke`, `docs/RELEASE_PLAN.md` |
