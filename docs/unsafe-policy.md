...
Limitations: early single-core terminal boot smoke only; no return to general post-switch services yet, no TLB/PCID handling yet, no per-core/per-task stack guards yet, and no reclamation of Limine page tables yet; before SMP, the static activation arena and stack should move to an explicit interior-mutability wrapper such as `SyncUnsafeCell` or an equivalent per-core ownership model
```
