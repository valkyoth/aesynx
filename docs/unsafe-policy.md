...
Limitations: early single-core terminal boot smoke only; no return to general post-switch services yet, no TLB/PCID handling yet, no per-core/per-task stack guards yet, and no reclamation of Limine page tables yet
```
