...
Limitations: bounded static heap only; page-backed means page-sized runs inside the static kernel heap, not physical-frame-backed growth from the global frame allocator; one global allocator lock remains; the backing heap is still a `static mut` raw-address pattern that must move to explicit interior mutability or an equivalent ownership wrapper before SMP; the standard `GlobalAlloc::dealloc(ptr, layout)` ABI cannot distinguish a delayed stale raw-pointer free from the current owner freeing the same address after reuse, so allocation-epoch ownership tokens or quarantine remain future work; no per-core heaps, allocation-while-locking policy, bounded IRQ-masked latency policy for materially larger heaps, backtrace leak reports, or full SMP allocator synchronization policy yet
```
