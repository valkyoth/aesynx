...
Status: v0.21.0 Memory capability candidate
...
`cargo xtask image` creates `build/qemu/aesynx-v0.21.0.iso` with Limine and the
...
`[TEST] memory-cap=ok`, and `[TEST] kernel-cr3=ok`.

`cargo xtask qemu --panic-smoke` creates a separate
`build/qemu/aesynx-v0.21.0-panic.iso`, enables the kernel `panic-smoke` feature,
and expects `[TEST] idt=ok`, `[TEST] irq=ok`, `[TEST] exception=ok`, and
`[TEST] panic=ok`.

`cargo xtask qemu --exception-smoke` creates a separate
`build/qemu/aesynx-v0.21.0-exception.iso`, enables the kernel
...
`build/qemu/aesynx-v0.21.0-timer.iso`, enables the kernel `timer-smoke` feature,
...
in sequence and is the GitHub CI QEMU gate for v0.21.
...
The v0.21 image proves that Limine can load the Rust kernel ELF, reach `_start`,
...
`[TEST] cap=ok`. The v0.21 candidate then gates mapper-facing checked mapping
...
The v0.21.0 candidate extends the kernel-facing capability model in
`aesynx-cap` with `MemoryMapRequest`, memory access classes, and memory mapping
authorization based on capability kind, bounded range, `MAP`, and
