...
`cargo xtask image` creates `build/qemu/aesynx-v0.35.2.iso` with Limine and the
...
```bash
cargo xtask trace-decode build/qemu/aesynx-v0.35.2.serial.log
```

`cargo xtask qemu --panic-smoke` creates a separate
`build/qemu/aesynx-v0.35.2-panic.iso`, enables the kernel `panic-smoke` feature,
and expects `[TEST] idt=ok`, `[TEST] irq=ok`, `[TEST] exception=ok`, and
`[TEST] panic=ok`.

`cargo xtask qemu --exception-smoke` creates a separate
`build/qemu/aesynx-v0.35.2-exception.iso`, enables the kernel
...
`build/qemu/aesynx-v0.35.2-timer.iso`, enables the kernel `timer-smoke` feature,
...
`barrier_ok` before `[TEST] amp-core=ok`. The v0.35.2 candidate runs QEMU with
...
shared-bootstrap-only descriptors as an explicit execution blocker, and QEMU
reports `startup_evidence_ok=true`, `ap_preflight_ok=true`, and
`ap_execution_blocked_ok=true` before `[TEST] multicore-topology=ok`. This does
not enable AP execution; the existing
