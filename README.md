...
`v0.21.0` is the current memory capability candidate.
...
| Capability model | Active candidate | `v0.21.0`; private non-copy authority values, checked `CapId` slot/generation layout, fixed-capacity kernel capability table, permission validation, audited derive/grant paths, slot generation stale-id rejection, revoke authority checks, redacted capability/table debug output, and memory-map authorization based on capability kind, range, and `MAP`/READ/WRITE/EXECUTE permissions. |
...
```bash
cargo xtask release-ready v0.21.0
```
...
- [v0.21.0 Release Candidate Notes](docs/releases/v0.21.0-rc.md)
