...
checks, kernel-range policy checks, user-range policy checks, write-protected range checks,
...
fail-closed leaf decoding for lookup/protect/unmap, translate, consistency
audit, empty-table reclamation, and explicit TLB flush targets before `[TEST]
page-table=ok`. The opt-in timer smoke
