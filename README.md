...
translate, unmap, and explicit TLB flush targets before `[TEST] page-table=ok`. The opt-in timer smoke
...
| Page table mapper | Active candidate | `v0.15.0`; safe bounded `aesynx-mm` page-table mapper model with x86_64-shaped tables, map/unmap/translate, permission lookup, explicit TLB flush targets, and QEMU smoke with `[TEST] page-table=ok`. |
