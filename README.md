...
change, unmapped range checks, translate, unmap, empty-table reclamation, and
...
| Page table mapper | Active candidate | `v0.15.0`; safe bounded `aesynx-mm` page-table mapper model with x86_64-shaped tables, map/unmap/translate, contiguous range map/protect/unmap plus lookup, unmapped range checks, permission lookup/change, empty-table reclamation, explicit TLB flush targets, and QEMU smoke with `[TEST] page-table=ok`. |
