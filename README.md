...
- The multicore topology smoke runs under QEMU `-smp 4` and models four visible
  ...
  Rust code yet.
- The IPC fabric smoke routes a kernel-stamped Ping from core 0 to core 1,
  ...
  `backpressure_ok=true` before `[TEST] ipc-pingpong=ok`.
...
| Core-to-core ping/pong | Active candidate | `v0.36.0`; no_std `aesynx-ipc` now models pairwise SPSC fabric links, kernel-stamped Ping/Pong messages, monotonic sequence numbers, route validation, release/acquire ordering evidence, and fail-closed backpressure. QEMU requires `ipc-pingpong ping_seq=`, `backpressure_ok=true`, `release_acquire_ok=true`, `pairwise_route_ok=true`, and `[TEST] ipc-pingpong=ok` after the multicore topology smoke. |
