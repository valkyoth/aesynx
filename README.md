...
| Kernel-owned address space | In progress | `v0.16.2`; audited mapper state now exports to a redacted x86_64 hardware-shaped page-table image, the root comes from a checked frame-allocator path, and QEMU requires `hardware_image_ok=true`; the actual CR3 switch is still remaining release work. |
