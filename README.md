...
`v0.29.0` is the current scheduler telemetry baseline candidate.
...
| Scheduler telemetry | Active candidate | `v0.29.0`; bounded scheduler decision records, deterministic round-robin decision reasons, task scheduled-run counters, core run-queue telemetry, redacted decision debug output, and QEMU `[TEST] scheduler-telemetry=ok`. |
...
```bash
cargo xtask release-ready v0.29.0
```
...
- [v0.29.0 Release Candidate Notes](docs/releases/v0.29.0-rc.md)
