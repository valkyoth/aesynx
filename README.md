...
| QEMU first boot | Active | `cargo xtask image` creates a release-profile Limine ISO and `cargo xtask qemu` verifies descriptor/IRQ setup, checked memory-map/frame-allocator/page-table markers, every v0.16 paging-policy-model `*_ok=true` marker, `[TEST] paging-policy-model=ok`, `[TEST] bootinfo=ok`, and `[TEST] boot=ok` from Rust `_start`. |
...
| Kernel-owned address space | In progress | `v0.16.2`; audited mapper state now streams redacted x86_64 hardware-shaped page-table entries using Limine's normalized kernel physical placement, keeps a checked frame-allocator root proof, copies used tables into a static activation arena, and QEMU requires `hardware_image_ok=true` plus `hardware_copied=true`; the actual CR3 switch is still remaining release work. |
...
| Area | Status | Target |
| --- | --- | --- |
| Kernel-owned address space | In progress | `v0.16.2`; finish the CR3 load, post-switch smoke evidence, and final pentest gate. |
| CPU hardening and stack guards | Planned | `v0.16.3`; enable NX/WP and gated SMEP/SMAP/UMIP where supported, then guard kernel stacks. |
