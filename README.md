...
`v0.27.0` is the current kernel task model candidate.
...
| Kernel task model | Active candidate | `v0.27.0`; no_std `aesynx-sched` task objects, checked state transitions, redacted task debug output, fixed-capacity local run queues, message/timer/object wait queues, fail-closed queue admission, and QEMU `[TEST] task-model=ok`. |
...
```bash
cargo xtask release-ready v0.27.0
```
...
- [v0.27.0 Release Candidate Notes](docs/releases/v0.27.0-rc.md)
