...
map/unmap, permission lookup, permission change, translate, unmap, empty-table
...
| Page table mapper | Active candidate | `v0.15.0`; safe bounded `aesynx-mm` page-table mapper model with x86_64-shaped tables, map/unmap/translate, contiguous range map/unmap, permission lookup/change, empty-table reclamation, explicit TLB flush targets, and QEMU smoke with `[TEST] page-table=ok`. |
